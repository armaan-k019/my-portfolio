...
function getBenchmarkRating(perM2: number, buildingType: string): { label: string; color: string; tier: string } {
  ...
  if (!b) return { label: "Select building type", color: "#9B8E85", tier: "unknown" };
  ...
  return                           { label: "High carbon — significant intervention needed", color: "#C1513A", tier: "poor" };
}
...
function BreakdownChart({ rows }: { rows: MaterialRow[] }) {
  ...
  return (
    <div className="space-y-2">
      {byCategory.map(([cat, val]) => {
        ...
        return (
          <div key={cat} className="flex items-center gap-3 text-xs">
            <span className="w-24 shrink-0 text-[#6B6054] text-right truncate">{cat}</span>
            <div className="flex-1 flex items-center" style={{ minHeight: 20 }}>
              {isNeg ? (
                ...
                <div className="flex items-center">
                  <div style={{ width: pct, height: 14, backgroundColor: color, borderRadius: 3, opacity: 0.85 }} />
                  <span className="text-[10px] text-[#6B6054] ml-1.5">{fmtCarbon(val)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flex items-center gap-3 text-[10px] text-[#9B8E85] mt-2 border-t border-[#E8E0D4] pt-2">
...
function BenchmarkGauge({ perM2, buildingType }: { perM2: number | null; buildingType: string }) {
  if (!perM2 || !buildingType || !BENCHMARKS[buildingType]) {
    return (
      <p className="text-xs text-[#9B8E85] italic">
  ...
  const tiers = [
    ...
    { to: 100,                            color: "#C1513A", label: `>${b.poor}` },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative h-4 rounded-full overflow-hidden flex">
        ...
        {/* needle */}
        <div
          className="absolute top-0 bottom-0 w-0.5 bg-[#2C1810]"
          style={{ left: `${pct}%`, transition: "left 0.5s" }}
        />
      </div>
      {/* Labels */}
      <div className="flex justify-between text-[9px] text-[#9B8E85]">
      ...
      <p className="text-[10px] text-[#9B8E85]">
        Your project: <strong className="text-[#2C1810]">{fmtNum(perM2)} kgCO₂e/m²</strong>
...
function SwapSuggestions({ rows, onSwap }: {
  ...
  if (swaps.length === 0 && optimized.length === 0) return (
    <p className="text-xs text-[#9B8E85] italic">No substitution opportunities identified for current materials.</p>
  );

  return (
    <div className="space-y-4">
      {swaps.length === 0 && (
        <p className="text-xs text-[#9B8E85] italic">No further substitution opportunities — all materials are at their lowest available carbon intensity.</p>
      )}
      <div className="space-y-2">
      {swaps.slice(0, 6).map((s, i) => (
        <div key={i} className="rounded-xl border border-[#E8E0D4] bg-white/60 p-3 space-y-2">
          <div className="flex items-start justify-between gap-2">
            <div className="space-y-0.5 flex-1 min-w-0">
              <p className="text-[10px] text-[#9B8E85] uppercase tracking-wide font-medium">Current</p>
              <p className="text-xs text-[#2C1810] font-medium truncate">{s.currentName}</p>
              <p className="text-[10px] text-[#9B8E85]">{s.currentIntensity.toLocaleString()} kgCO₂e/{CARBON_DATA[rows.find(r => r.id === s.rowId)?.materialKey ?? ""]?.unit}</p>
            </div>
            <div className="text-[#9B8E85] text-sm mt-3">→</div>
            <div className="space-y-0.5 flex-1 min-w-0 text-right">
              ...
              <p className="text-xs text-[#2C1810] font-medium truncate">{s.altName}</p>
              <p className="text-[10px] text-emerald-600">{s.altIntensity.toLocaleString()} kgCO₂e/{CARBON_DATA[s.altKey]?.unit}</p>
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-[#E8E0D4] pt-2">
            {s.altIntensity < 0 ? (
      ...
      {optimized.length > 0 && (
        <div className="mt-2">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85] mb-2">Already optimized</p>
            ...
            {optimized.map((m, i) => (
              <div key={i} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-emerald-50/60 border border-emerald-100">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs text-[#2C1810] font-medium">{m.name}</span>
                  <span className="text-[10px] text-emerald-600 ml-1.5">— lowest carbon option in category</span>
                </div>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-[#9B8E85] mt-2">
            <strong className="text-[#6B6054]">{optimized.length}</strong> of{" "}
            <strong className="text-[#6B6054]">{rows.length}</strong> materials are already at their lowest available carbon intensity
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-5">
      {sections.map((section, i) => {
        ...
        return (
          <div key={i}>
            {i > 0 && <div className="border-t border-[#E8E0D4] mb-4" />}
            {heading && (
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85] mb-2">{heading}</p>
            )}
            <div
              className="text-xs text-[#2C1810] leading-relaxed report-body"
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8]">
      <style>{`
        ...
        .report-body strong { font-weight: 600; color: #2C1810; }
      `}</style>

      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="border-b border-[#2C1810]/[0.08] bg-white/60 backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/#projects" className="text-xs text-[#9B8E85] hover:text-[#6B5244] transition-colors shrink-0">
            ← Back
          </Link>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg font-semibold text-[#2C1810] leading-tight">Carbon Lens</h1>
            ...
            <p className="text-xs text-[#9B8E85] mt-0.5">Embodied carbon estimator for early-stage architectural design</p>
          </div>
          <button
            onClick={exportCSV}
            className="shrink-0 text-xs px-3 py-1.5 rounded-lg border border-[#E8E0D4] text-[#6B6054] hover:border-[#C1513A] hover:text-[#C1513A] transition-colors bg-white/80"
          >
            Export CSV
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* ── Info banner ────────────────────────────────────────────────────── */}
        <div className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm p-5 grid md:grid-cols-2 gap-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85] mb-2">What is embodied carbon?</p>
          ...
          <div className="border-l border-[#E8E0D4] pl-6">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85] mb-2">Data source</p>
            <p className="text-xs text-[#6B6054] leading-relaxed">
              Carbon intensities from the <strong className="text-[#2C1810]">ICE Database v3.0</strong> (Inventory of Carbon
        ...
        {/* ── Project info ───────────────────────────────────────────────────── */}
        <section className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8E0D4]">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">Project Information</p>
          </div>
          <div className="p-5 grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="lg:col-span-2">
              <label className="block text-[10px] font-medium text-[#9B8E85] uppercase tracking-wide mb-1">Project Name</label>
              <input
                type="text"
                placeholder="e.g. Riverside Housing Block"
                value={projectInfo.name}
                onChange={e => setProjectInfo(p => ({ ...p, name: e.target.value }))}
                className="w-full px-3 py-2 text-xs border border-[#E8E0D4] rounded-lg focus:outline-none focus:border-[#C1513A] bg-white text-[#2C1810] placeholder:text-[#9B8E85]"
              />
            </div>
            <div>
              <label className="block text-[10px] font-medium text-[#9B8E85] uppercase tracking-wide mb-1">Building Type</label>
              <select
                value={projectInfo.buildingType}
                onChange={e => setProjectInfo(p => ({ ...p, buildingType: e.target.value }))}
                className="w-full px-3 py-2 text-xs border border-[#E8E0D4] rounded-lg focus:outline-none focus:border-[#C1513A] bg-white text-[#2C1810]"
              >
                {BUILDING_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[10px] font-medium text-[#9B8E85] uppercase tracking-wide mb-1">Gross Floor Area (m²)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-[10px] font-medium text-[#9B8E85] uppercase tracking-wide mb-1">Location</label>
              <input
                type="text"
        ...
        <section className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8E0D4] flex items-center justify-between">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">Material Schedule</p>
            <button
              onClick={addRow}
              className="text-[10px] px-3 py-1 rounded-lg bg-[#C1513A]/10 text-[#C1513A] font-medium hover:bg-[#C1513A]/20 transition-colors border border-[#C1513A]/20"
            >
              + Add material
            </button>
          </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#E8E0D4] bg-[#F5F0E8]/50">
                  <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-wide text-[#9B8E85] w-72">Material</th>
                  <th className="text-left px-3 py-3 text-[10px] font-semibold uppercase tracking-wide text-[#9B8E85] w-28">Quantity</th>
                  <th className="text-left px-3 py-3 text-[10px] font-semibold uppercase tracking-wide text-[#9B8E85] w-16">Unit</th>
                  <th className="text-left px-3 py-3 text-[10px] font-semibold uppercase tracking-wide text-[#9B8E85] w-36">kgCO₂e / unit</th>
                  <th className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wide text-[#9B8E85]">Total kgCO₂e</th>
                  <th className="w-10" />
                </tr>
              </thead>
              <tbody className="divide-y divide-[#E8E0D4]/60">
                {rows.map((row, i) => {
                  const mat = CARBON_DATA[row.materialKey];
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-[#F5F0E8]/30 transition-colors group">
                      <td className="px-5 py-3">
                        <select
                          value={row.materialKey}
                          onChange={e => updateRow(row.id, "materialKey", e.target.value)}
                          className="w-full text-xs border-0 focus:outline-none bg-transparent text-[#2C1810] cursor-pointer"
                        >
                        ...
                        {mat && <p className="text-[10px] text-[#9B8E85] mt-0.5 leading-tight">{mat.description}</p>}
                      </td>
                      <td className="px-3 py-3">
                        <input
                          ...
                          className="w-24 px-2 py-1 text-xs border border-[#E8E0D4] rounded focus:outline-none focus:border-[#C1513A] bg-white text-[#2C1810] placeholder:text-[#9B8E85]"
                        />
                      </td>
                      <td className="px-3 py-3 text-[#9B8E85]">{mat?.unit}</td>
                      <td className="px-3 py-3">
                        <span className={`font-mono text-[11px] ${mat && mat.kgCO2e_per_unit < 0 ? "text-emerald-600" : "text-[#6B6054]"}`}>
                      ...
                      <td className="px-5 py-3 text-right">
                        {hasQty ? (
                          <span className={`font-mono text-xs font-medium ${isNeg ? "text-emerald-600" : "text-[#2C1810]"}`}>
                            {isNeg && "🌿 "}{fmtCarbon(lineTotal)}
                          </span>
                        ) : (
                          <span className="text-[#9B8E85] text-[10px]">—</span>
                        )}
                      </td>
                      <td className="pr-3">
                        <button
                          onClick={() => removeRow(row.id)}
                          className="opacity-0 group-hover:opacity-100 text-[#9B8E85] hover:text-red-500 transition-all text-base leading-none"
                        >
                          ×
                        </button>
                      </td>
          ...
          {/* Mobile cards */}
          <div className="md:hidden divide-y divide-[#E8E0D4]/60">
            {rows.map((row, i) => {
              const mat = CARBON_DATA[row.materialKey];
              const lineTotal = byRow[i] ?? 0;
              const isNeg = lineTotal < 0;
                ...
                <div key={row.id} className="p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <select
                      value={row.materialKey}
                      onChange={e => updateRow(row.id, "materialKey", e.target.value)}
                      className="flex-1 text-xs border border-[#E8E0D4] rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#C1513A] bg-white text-[#2C1810]"
                    >
                      {Object.entries(MATERIAL_GROUPS).map(([cat, items]) => (
                        <optgroup key={cat} label={cat}>
                    ...
                    <button onClick={() => removeRow(row.id)} className="text-[#9B8E85] hover:text-red-500 transition-colors shrink-0">×</button>
                  </div>
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      value={row.quantity}
                      onChange={e => updateRow(row.id, "quantity", e.target.value)}
                      className="w-28 px-2 py-1 text-xs border border-[#E8E0D4] rounded focus:outline-none focus:border-[#C1513A] bg-white text-[#2C1810]"
                    />
                    <span className="text-[10px] text-[#9B8E85]">{mat?.unit}</span>
                    <span className="ml-auto text-xs font-medium font-mono">
                      <span className={isNeg ? "text-emerald-600" : "text-[#2C1810]"}>
          ...
          {/* Totals footer */}
          <div className="border-t border-[#E8E0D4] px-5 py-4 bg-[#F5F0E8]/40 space-y-3">
            <div className="flex flex-wrap items-center gap-x-8 gap-y-2">
              <div>
                <p className="text-[10px] text-[#9B8E85] uppercase tracking-wide font-medium">Total Embodied Carbon</p>
                <p className={`text-xl font-semibold mt-0.5 ${totalCarbon < 0 ? "text-emerald-600" : "text-[#2C1810]"}`}>
                  {totalCarbon < 0 && "🌿 "}{fmtCarbon(totalCarbon)}
                </p>
              </div>
              {perM2 !== null && (
                <div>
                  <p className="text-[10px] text-[#9B8E85] uppercase tracking-wide font-medium">Per m²</p>
                  <p className={`text-xl font-semibold mt-0.5 ${perM2 < 0 ? "text-emerald-600" : "text-[#2C1810]"}`}>
            ...
            {totalCarbon !== 0 && (
              totalCarbon < 0 ? (
                ...
                <p className="text-[10px] text-[#9B8E85]">
                  Equivalent to&nbsp;
                  <strong className="text-[#6B6054]">{flights} transatlantic flights</strong>
                  &nbsp;or&nbsp;
                  <strong className="text-[#6B6054]">{carYears} years</strong>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E8E0D4]">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">Carbon by Category</p>
            </div>
            <div className="p-5">
              {rows.every(r => !parseFloat(r.quantity)) ? (
                <p className="text-xs text-[#9B8E85] italic">Enter material quantities above to see the breakdown.</p>
              ) : (
          ...
          <div className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E8E0D4]">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">Benchmark Comparison</p>
            </div>
            <div className="p-5">
        ...
        <section className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8E0D4]">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">Material Substitution Opportunities</p>
            <p className="text-[10px] text-[#9B8E85] mt-0.5">Ranked by potential carbon saving — biggest wins first</p>
          </div>
          <div className="p-5">
        ...
        {/* ── AI report ─────────────────────────────────────────────────────── */}
        <section ref={reportSectionRef} className="bg-white/80 rounded-xl border border-[#E8E0D4] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8E0D4] flex items-center justify-between gap-3 flex-wrap">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9B8E85]">AI Consultant Report</p>
              <p className="text-[10px] text-[#9B8E85] mt-0.5">Expert recommendations generated by Claude</p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {aiReport && (
                <button
                  onClick={copyReport}
                  className="text-xs px-3 py-2 rounded-lg border border-[#E8E0D4] text-[#6B6054] hover:border-[#C1513A] hover:text-[#C1513A] transition-colors bg-white/80"
                >
              ...
              <button
                ...
                className="text-xs px-4 py-2 rounded-lg bg-[#C1513A] text-white font-medium hover:bg-[#a8412c] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
          ...
          <div className="p-5">
            ...
            {aiLoading && (
              <div className="flex items-center gap-2 text-xs text-[#9B8E85]">
                <span className="w-3 h-3 rounded-full border-2 border-[#C1513A]/30 border-t-[#C1513A] animate-spin" />
            ...
            {aiReport && (
              <div>
                ...
                <p className="text-[10px] text-[#9B8E85]/60 text-right mt-4">Generated by Claude (Anthropic)</p>
              </div>
            )}
          </div>
        </section>
        ...
        {/* ── Legend ────────────────────────────────────────────────────────── */}
        <div className="pb-8">
          <p className="text-[10px] text-[#9B8E85]/70 leading-relaxed text-center max-w-2xl mx-auto">
