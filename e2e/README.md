...
Its 21 points are randomised per run as well, within a fixed rural cell in western Kansas, so every
request creates a site row and is chargeable. Those two together are why repeats are safe: the
points and the IP are both random per run, so nothing an earlier run left behind can make one of
this run's requests free. It asserts exactly 20 responses with status 200 and `rateLimit.remaining`
counting down 19 to 0, then a 429 with `resetAt` on the twenty first.
...
`src/lib/datum/memory.ts`, since the salt is not exported) and the `sites` rows for the 21 site keys
it generated (computed with `siteKey` from `src/lib/datum/geo.ts`), not every `is_test` row in the
project. It prints row counts only, never a URL and never a key. The SQL below stays as the manual
fallback for a run that was interrupted or run without `DATUM_E2E_DB=1`.
