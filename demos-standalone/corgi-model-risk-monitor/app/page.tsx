...
export default function CorgiPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch("https://armaankazi.com/api/corgi/recommendations", {
        method: "POST",
      ...
      <header style={{ backgroundColor: C.headerBg, borderBottomColor: C.headerBorder }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-xs leading-relaxed max-w-2xl" style={{ color: "#94a3b8" }}>
          ...
          <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
            <span className="text-xs" style={{ color: "#fbbf24" }}>
              Demo by{" "}
              <a href="https://armaankazi.com" className="hover:underline" style={{ color: "#fbbf24" }}>Armaan Kazi</a>
            </span>
          </div>
        </div>
      </header>
            ...
            <div className="rounded-2xl p-6 sm:p-8" style={{ backgroundColor: C.headerBg }}>
              ...
              {recsLoading && !recs && (
                <div className="flex items-center gap-3 py-4">
                  ...
                  <span className="text-sm" style={{ color: "#94a3b8" }}>Analyzing coverage against drift metrics…</span>
                </div>
              )}

              {recsError && (
                <div className="rounded-xl border px-4 py-3 text-sm" style={{ backgroundColor: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.10)", color: "#94a3b8" }}>
                  {recsError}
                </div>
              )}
              ...
              {recs && (
                <div className="space-y-5">
                  ...
                  {/* Risk factors */}
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: "#60a5fa" }}>Risk Factors Identified</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {recs.risk_factors.map((f, i) => (
                        <div key={i} className="rounded-xl border p-3" style={{ backgroundColor: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.10)" }}>
                          <p className="text-xs leading-relaxed" style={{ color: "#cbd5e1" }}>{f}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Recommendations */}
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: "#60a5fa" }}>Recommended Actions</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {recs.recommendations.map((r, i) => {
                        const urg = URGENCY_STYLES[r.urgency] ?? URGENCY_STYLES.Medium;
                        return (
                          <div key={i} className="rounded-xl border p-4" style={{ backgroundColor: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.10)" }}>
                            ...
                            <p className="text-[11px] leading-relaxed" style={{ color: "#cbd5e1" }}>{r.explanation}</p>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {/* Regulatory note */}
                  <div className="flex items-start gap-2 rounded-xl border px-3 py-2.5" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.10)" }}>
                    <span className="text-sm shrink-0">⚖️</span>
                    <p className="text-[11px] leading-relaxed" style={{ color: "#94a3b8" }}>{recs.regulatory_note}</p>
                  </div>
                </div>
              )}
            </div>
